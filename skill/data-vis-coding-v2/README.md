...
> 此外，`v2` 版本支援透過內建腳本（`scripts/scaffold.py`）一鍵生成專案骨架（Scaffold）（相容純前端或全端架構，並內建雙主題切換），同時提供版面選擇器（Layout Picker）（`assets/layout-picker-template.html`）供初期開發階段進行空間佈局之評估與決策。

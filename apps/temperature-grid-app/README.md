...
右=ridgeline plot(各月網格月均溫分布,點縣市時切為該縣),底部月份播放軸同步驅動。
方向 A 雙主題(預設深色)+中英雙語;溫度色階以老師 color-matrix 的 LCH 方法生成
...
- `npm test` — vitest 12 項:KDE(積分=1/峰位/雙峰保留)、最近鄰查詢、色階端點與單調性
- `npm run build` — tsc --noEmit + vite build
- Playwright 實測:月份切換/播放前進/hover tooltip(數值對照原始 CSV 命中)/深色模式,截圖見 docs/screenshots/

## 下一階段

- 年份播放(跨年動畫;月份播放已具備)+ridgeline 跨年 morph(參考 d3-graph-gallery ridgeline_animation)
- 部署前評估 temps JSON 量化壓縮(45 檔×~0.8MB)

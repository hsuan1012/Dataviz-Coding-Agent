...
export const themes = {
  light: {
    ...
    // 本專案：22 縣市循環使用 8 色（對齊頂層 categoricalPalette 全 8 色）
    categorical: ['#2A6F97', '#0D9488', '#C77D3E', '#6A8E3C', '#8C5C9E', '#457B9D', '#B5651D', '#5F8D4E'],
    // treemap LCH 配色（老師 8/14 調色盤 app：d3.hcl 色相環×明度階、彩度 45）
    // 22 縣市＝色相環均分（hue = index×360/22）；群組標題帶=stripL、葉子依序循環 leafL 明度階
    // chroma 45→30：使用者 8/14 回饋太鮮豔，降彩度走柔和路線（色相/明度階不變）
    // ratioL：老師 8/17 回饋③＝葉子明度改對應生師比（比值小→light 端、大→dark 端）；leafL 保留供對照
    treemapLch: { chroma: 30, stripL: 44, leafL: [58, 66, 74], ratioL: { light: 78, dark: 48 } },
  ...
  dark: {
    ...
    // 本專案：22 縣市循環使用 8 色（後 3 色為淺色版 8 色的提亮對應）
    categorical: ['#5FA8D3', '#2DD4BF', '#E0A567', '#9CC06B', '#B98FC9', '#79A8C9', '#DE9557', '#8FBC77'],
    // treemap LCH 配色（深色主題：彩度略降、明度整體下移，維持深底對比）
    // ratioL 同淺色主題語意：生師比小→light 端（深底下仍是「越小越淺」）
    treemapLch: { chroma: 26, stripL: 30, leafL: [42, 50, 58], ratioL: { light: 64, dark: 32 } },

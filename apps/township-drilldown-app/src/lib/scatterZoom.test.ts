...
import { zoomDomain, panDomain, zoomDomainInSpace, panDomainInSpace, symlogFwd, symlogInv } from "./scatterZoom";
...
// 8/7 對數空間縮放(使用者定案,取代方案 A):symlog/log 軸縮放中保留原刻度型態,
// 縮放數學改在「刻度轉換空間」做——轉換空間的值比例=像素比例,錨點像素級精準;
// 值空間版在寬 log 軸上第一格會因 clamp 把游標下的點推走半個繪圖區(gapminder 實測)。
describe("zoomDomainInSpace(轉換空間縮放)", () => {
  const C = 2000;
  const fwd = symlogFwd(C);
  const inv = symlogInv(C);

  it("symlogFwd/symlogInv 互為反函數(含 0 與負值)", () => {
    for (const v of [0, 1, 500, 2000, 9731.6, 40000, -300]) {
      expect(inv(fwd(v))).toBeCloseTo(v, 8);
    }
  });

  it("symlog 空間放大:錨點在轉換空間的比例不變(=游標像素位置不動)", () => {
    const full: [number, number] = [0, 40000];
    const anchor = 5000;
  ...
  it("縮小超過全域寬度 → 回 null(還原全範圍)", () => {
    const d = zoomDomainInSpace([1000, 8000], [0, 40000], 3000, 0.1, fwd, inv);
    expect(d).toBeNull();
  });
});
...
describe("panDomainInSpace(轉換空間平移)", () => {
  ...
  it("symlog 空間平移 delta(轉換空間單位),寬度(轉換空間)不變", () => {
    const cur: [number, number] = [1000, 8000];
    const d = panDomainInSpace(cur, [0, 40000], 0.3, fwd, inv);
    expect(fwd(d[1]) - fwd(d[0])).toBeCloseTo(fwd(cur[1]) - fwd(cur[0]), 10);
    expect(fwd(d[0])).toBeCloseTo(fwd(cur[0]) + 0.3, 10);
  });

  it("平移貼到全域邊界 → clamp(轉換空間寬度不變)", () => {
    const cur: [number, number] = [1000, 8000];
    const d = panDomainInSpace(cur, [0, 40000], -99, fwd, inv);
    expect(d[0]).toBeCloseTo(0, 8);
    expect(fwd(d[1]) - fwd(d[0])).toBeCloseTo(fwd(cur[1]) - fwd(cur[0]), 10);
  });
